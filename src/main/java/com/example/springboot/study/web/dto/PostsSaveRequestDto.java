...
B001 : 게시글을 저장하는 Request DTO
    ...
    Request/Response용 Dto는 View를 위한 클래스이기 때문에 자주 변경이 필요한 경우가 많다.
    View Layer와 DB Layer의 역할 분담하는 것이 바람직하다.

    Controller에서 결과값을 여러 테이블을 조인해햐 하는 경우가 빈번하기 때문에
    Entity 클래스 만으로 표현하기 어려운 경우가 종종 발생한다.

    따라서!!!!
    꼭 Entity Class와 Controller에서 사용할 DTO를 분리하자.

    Controller를  먼저 만들면 어떤 문제가 발생하는지 확인
